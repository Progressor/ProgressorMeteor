(function () {
	...
	_.extend(Progressor, {
		...
		sendReleaseNotification() {
			const requestedExercises = Progressor.exercises.find({ 'released.requested': { $exists: true }, 'released.confirmed': { $exists: false }, 'released.notified': { $exists: false } }).fetch();
			if (requestedExercises.length) {
				Meteor.users.find({ roles: Progressor.ROLE_ADMIN }).forEach(i18nForUser(user => {
					Email.send(
						{
				...
				_.each(requestedExercises, exercise => Progressor.exercises.update({ _id: exercise._id }, { $set: { 'released.notified': new Date() } }));
			}
		}
	});
	...
	Accounts.emailTemplates.verifyEmail = {
		...
		html: i18nForUser((user, url) => SSR.render('verifyEmail', { address: Progressor.getUserName(user, true) || Progressor.getUserEmail(user), verifyURL: url }))
	};
	...
	Accounts.emailTemplates.resetPassword = {
		...
		html: i18nForUser((user, url) => SSR.render('resetPassword', { address: Progressor.getUserName(user, true) || Progressor.getUserEmail(user), resetpwdURL: url }))
	};
})();
