(function () {
	...
	Router.route('/account', {
		...
		data() {
			...
			return {
				...
				createdExaminations: _.map(Progressor.examinations.find({ author_id: Meteor.userId() }, { sort: [['lastEdited', 'desc']] }).fetch(), Progressor.joinCategory),
				createdExecutions: _.map(Progressor.executions.find({ author_id: Meteor.userId() }, { sort: [['lastEdited', 'desc']] }).fetch(), Progressor.joinCategory)
			};
		}
	});
	...
	Router.route('/exercise/:_id', {
		...
		data() {
			const exercise = Progressor.exercises.findOne();
			if (exercise) return Progressor.joinExecution(Progressor.joinCategory(exercise));
		}
	});
	...
	Router.route('/examination/:_id', {
		...
		waitOn() {
			return [
				...
				Meteor.subscribe('resultsByExecution', this.params._id, true),
				Meteor.subscribe('execution', this.params._id)
			];
		}, data() {
			return Progressor.executions.findOne();
