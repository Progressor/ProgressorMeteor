...
Router.route('/account', {
  ...
  data() {
    ...
    return {
      results:
        _.map(
          Progressor.results.find({}, { sort: [['solved', 'desc']] }).fetch(),
          r => _.extend({}, r, { exercise: Progressor.joinCategory(r.exercise) })
        ),
      createdExercises:
        _.map(
          Progressor.exercises.find({ author_id: Meteor.userId(), archived: { $not: true } }, { sort: [['lastEdited', 'desc']] }).fetch(),
          Progressor.joinCategory
        ),
      archivedExercises:
        _.map(
          Progressor.exercises.find({ author_id: Meteor.userId(), archived: true }, { sort: [['lastEdited', 'desc']] }).fetch(),
          Progressor.joinCategory
        ),
      createdExaminations:
        Progressor.examinations.find({ author_id: Meteor.userId(), archived: { $not: true } }, { sort: [['lastEdited', 'desc']] }).fetch(),
      archivedExaminations:
        Progressor.examinations.find({ author_id: Meteor.userId(), archived: true }, { sort: [['lastEdited', 'desc']] }).fetch(),
      createdExecutions:
        Progressor.executions.find({ author_id: Meteor.userId(), archived: { $not: true } }, { sort: [['lastEdited', 'desc']] }).fetch(),
      archivedExecutions:
        Progressor.executions.find({ author_id: Meteor.userId(), archived: true }, { sort: [['lastEdited', 'desc']] }).fetch(),
    };
  },
});
...
Router.route('/category/:_id', {
  ...
  data() {
    const category = Progressor.categories.findOne();
    const exercises = Progressor.exercises.find().fetch();
    // if (exercises.length)
    if (category) {
      return {
        isCategory: true,
...
Router.route('/language/:_id', {
  ...
  data() {
    const programmingLanguage = _.find(Progressor.getProgrammingLanguages(), l => l._id === this.params._id);
    const exercises = Progressor.exercises.find().fetch();
    // if (exercises.length)
...
Router.route('/exercise/release', {
  ...
  data() {
    return {
      requested: _.map(
        Progressor.exercises.find({ 'released.confirmed': { $exists: false } }, { sort: [['released.requested', 'desc']] }).fetch(),
        Progressor.joinCategory
      ),
      released: _.map(
        Progressor.exercises.find({ 'released.confirmed': { $exists: true } }, { sort: [['released.requested', 'desc']] }).fetch(),
        Progressor.joinCategory
      ),
    };
  },
...
Router.route('/exercise/create', {
  ...
  data() {
    ...
    const type = exercise ? exercise.type : this.params.query.type;
    return type ? { type: parseInt(type, 10) } : {};
  },
});

Router.route('/exercise/import', {
  name: 'exerciseImport',
  onBeforeAction: ['authenticate'],
  authenticate: {
    template: 'account',
