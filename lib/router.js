(function () {
	...
	function serveFile(generator) {
		return function () {
			const { content, contentType, fileName = null, attachment = false } = generator.call(this);
			const dispositionFileName = fileName ? `; filename=${fileName}` : null;

			this.response.writeHead(200, {
				'Content-Type': contentType,
				'Content-Disposition': `${attachment ? 'attachment' : 'inline'}${dispositionFileName}`
			});
	...
	Router.route('/examination/:_id/pdf-empty', serveFile(function () {
		const examination = Progressor.executions.findOne({ _id: this.params._id });
		return {
			content: Progressor.generateExaminationPDF(examination).outputSync(),
			contentType: 'application/pdf',
			fileName: `${examination._id}.pdf`
		};
	}), { where: 'server' });
