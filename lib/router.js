(function () {
	...
	Router.route('/exercise/create', {
		...
		waitOn() {
			let sub = [Meteor.subscribe('categories')];
	...
	Router.route('/exercise/edit/:_id', {
		...
		data() {
			let exercise = Progressor.exercises.findOne();
			if (exercise)
				return Progressor.joinCategory(exercise);
		}
	});
	...
	Router.route('/result/:_id', {
		...
		data() {
			...
			if (result)
				return _.extend({}, result, { exercise: Progressor.joinCategory(result.exercise) });
		}
	});
