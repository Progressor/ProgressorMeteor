	...
	_.extend(Progressor, {
		...
		generateExecutionPDF(execution, includeResults = false) {
					...
					switch (exercise.type) {
						case 1: //programming
							...
							_.each(exercise.testCases, (testCase, testCaseIndex) => {
								...
								pdf.rect(testCaseX1, y1 + testCaseSize * testCaseIndex * 2, PAGE_SIZE.width - PAGE_SIZE.margin * 2 - testCaseX1, testCaseSize * 2).fillAndStroke('#FFFFFF').fillColor('#000000');
								pdf.fontRegular(testCaseSize).text(Progressor.getTestCaseSignature(exercise, testCase), testCaseX1 + FONT_SIZE / 2, y1 + testCaseSize * testCaseIndex * 2 + testCaseSize / 2,
								...
								pdf.fontRegular(testCaseSize).text(output, testCaseX2 + testCaseSize / 2, y1 + testCaseSize * testCaseIndex * 2 + testCaseSize / 2,
																									 { width: PAGE_SIZE.width - PAGE_SIZE.margin - testCaseX2 - testCaseSize, height: testCaseSize });
							});

							if (result || exercise.fragment)
								pdf.fontRegular().text(result.fragment || exercise.fragment, x1 + FONT_SIZE / 2, y1 + testCaseSize * exercise.testCases.length * 2 + FONT_SIZE / 2, { width: fragmentWidth - FONT_SIZE });
			...
			stream.on('finish', function () {
				downloadFile(this.toBlob('application/pdf'), `${execution._id}_${userTitle.replace(/[^a-zA-z0-9]+/g, '-')}.pdf`.toLowerCase());
			});
			pdf.end();
		},
		...
		generateExecutionCSV(execution) {
			...
			downloadFile(new Blob([csv], { type: 'text/csv' }), `${execution._id}_${i18n.getNameForLanguage(execution, i18n.getLanguageForUser(Meteor.user()), true).replace(/[^a-zA-z0-9]+/g, '-')}.csv`.toLowerCase());
		}
	});
